...
import (
	...
	"crypto/sha1"
	"encoding/hex"
	"goRssMail-go/pkg/util/goconv"
	"goRssMail-go/pkg/util/gostring"
	"goRssMail-go/pkg/util/gostruct"
	"sort"
	"strings"
)
...
func Getkv(v interface{}) (string, *bytes.Buffer) {
	...
	for _, k := range keys {
		if len(goconv.ToString(m[k])) > 0 {
			buf.WriteString(k)
			buf.WriteString(`=`)
			buf.WriteString(goconv.ToString(m[k]))
			buf.WriteString(`&`)
		}
	}

	kvs := string(buf.Bytes())
	kvs = gostring.Substr(kvs, 0, len(kvs)-1)
	return kvs, &buf
}
